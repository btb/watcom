 ...
 * LIFEMAIN.C
 *
 * Main line + actual windowing code for Windows life program
 *
 */
#include <string.h>
#include <stdio.h>
...
extern long _EXPORT FAR PASCAL WindowProc( HWND, unsigned, WORD, LONG );
...
static void InitMenu( WORD wparam )
/**********************************
...
*/
{
    ...
    CheckMenuItem( wparam, MENU_WRAP_AROUND, Check[ CurvedSpace ] );
    CheckMenuItem( wparam, MENU_BOUNDED_EDGES, Check[ !CurvedSpace ] );
    CheckMenuItem( wparam, MENU_PAUSE, Check[ Mode == MENU_PAUSE ] );
    CheckMenuItem( wparam, MENU_RESUME, Check[ Mode == MENU_RESUME ] );
    CheckMenuItem( wparam, MENU_SINGLE_STEP, Check[ MouseMode == MENU_SINGLE_STEP ]);
    CheckMenuItem( wparam, MENU_SELECT, Check[ MouseMode == MENU_SELECT ]);
    CheckMenuItem( wparam, MENU_FLIP_PATTERNS, Check[ MouseMode == MENU_FLIP_PATTERNS ]);
    EnableMenuItem( wparam, MENU_SAVE, Gray[ SelectOn() ]);
    CheckMenuItem( wparam, MENU_GRID, Check[ DrawGrid ]);
    for( i = 0; i < NumberPatterns; ++i ) {
	CheckMenuItem( wparam, MENU_PATTERN+i, Check[ IsCurrPattern( i ) ] );
    }
}
