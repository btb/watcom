...
static void DisplayAboutBox( win_handle )
/****************************************
...
*/
{
    ...
    inst_handle = GetWindowWord( win_handle, GWW_HINSTANCE );
    proc = MakeProcInstance( About, inst_handle );
    DialogBox( inst_handle, "AboutBox", win_handle, proc );
...
*/
{
    ...
    CheckMenuItem( wparam, MENU_SET_ALARM, Check[ Setting ] );
    CheckMenuItem( wparam, MENU_RUN_CLOCK, Check[ !Setting ] );
    on = ( NumberOfDigits == DIGITS_WITHOUT_SECONDS );
    CheckMenuItem( wparam, MENU_SUPRESS_SECONDS, Check[ on ] );
    CheckMenuItem( wparam, MENU_DISPLAY_SECONDS, Check[ !on ] );
    CheckMenuItem( wparam, MENU_24_HOUR_CLOCK, Check[ !TwelveHour ] );
    CheckMenuItem( wparam, MENU_12_HOUR_CLOCK, Check[ TwelveHour ] );
}
...
*/
{
    ...
    MoveTo( dc, pos->start.x, pos->start.y );
    while( *delta != 0 ) {
	if( *delta & START ) {
	    x = pos->start.x;
	    y = pos->start.y;
